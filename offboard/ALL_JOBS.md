...
All four are ~2,400 characters, the size of the rung that fails.

| packet | filler | fences | what it isolates |
|---|---|---|---|
| `packet_a.txt` | plain prose | 0 | **baseline** — can the model do this task at all |
| `packet_b.txt` | plain prose | 0 | **control** — a different marker count, catches a model repeating rather than counting |
| `packet_f.txt` | directive text | 10, balanced | **instruction collision** — does a body full of imperative rules stop the model obeying the last one |
| `packet_g.txt` | directive text | 11, **odd** | **unterminated fence** — the 500-token rung's actual defect, reproduced |

`a` and `f` differ in one thing: what the filler is made of. `f` and `g` differ in
one thing: whether the last code block is closed. That is the whole design.
